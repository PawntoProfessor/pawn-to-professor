...
import { defaultAppearance, defaultHomepage, demoPages, demoSections, hasSupabaseEnv } from '@/lib/config'
...
export default async function Home(){
  ...
  if(hasSupabaseEnv()){
    try{
      ...
      const [{data:hrow},{data:arow},{data:srows},{data:prows}]=await Promise.all([
      ...
      homepage={...defaultHomepage,...(hrow?.value||{})}
  ...
  return <main>
    <section className={`home-hero align-${appearance.heroTextAlign||'left'}`}><div className="wrap hero-inner"><div className="hero-copy"><span className="eyebrow">PAWN TO PROFESSOR</span><h1>{homepage.heroTitle}</h1><p>{homepage.heroText}</p><div className="hero-actions"><Link className="btn" href="/library/lessons">Explore resources</Link><Link className="btn secondary" href="/admin">Manage website</Link></div></div>{appearance.showHeroPiece!==false&&<div className="hero-art"><div className="hero-piece">♟️</div><div className="hero-bubble">Teach<br/>Play<br/>Inspire</div></div>}</div></section>
    <div className="wrap home-content">
      ...
      <section className="admin-callout"><div><span className="eyebrow">NO CODING NEEDED</span><h2>You control the website from the website.</h2><p>Create sections, add pages and games, change the design, hide old material, restore items from Trash, and upload media from your Admin area.</p></div><Link className="btn" href="/admin">Open Site Manager</Link></section>
    </div>
  </main>
}
